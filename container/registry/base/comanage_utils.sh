...
function comanage_utils::consume_injected_environment() {
    ...
    injectable_config_vars=( 
        ...
        COMANAGE_REGISTRY_DATABASE_DRIVER
        COMANAGE_REGISTRY_DATABASE_HOST
        COMANAGE_REGISTRY_DATABASE_PERSISTENT
        ...
        COMANAGE_REGISTRY_OIDC_PASS_CLAIMS_AS
        ...
        COMANAGE_REGISTRY_TRANSMOGRIFY_DATABASE_DRIVER
        COMANAGE_REGISTRY_TRANSMOGRIFY_DATABASE_HOST
        COMANAGE_REGISTRY_TRANSMOGRIFY_DATABASE_PERSISTENT
...
EOF
    ...
    php_string+=$'\n            '
    php_string+="'driver' => '${COMANAGE_REGISTRY_DATABASE_DRIVER:-Cake\Database\Driver\Postgres}',"

    php_string+=$'\n            '
    php_string+="'persistent' => ${COMANAGE_REGISTRY_DATABASE_PERSISTENT:-false},"

    php_string+=$'\n            '
    php_string+="'host' => '${COMANAGE_REGISTRY_DATABASE_HOST:-registry-database}',"

    php_string+=$'\n            '
    php_string+="'username' => '${COMANAGE_REGISTRY_DATABASE_USER:-registry_user}',"
    ...
    php_string+=$'\n            '
    php_string+="'encoding' => '${COMANAGE_REGISTRY_DATABASE_ENCODING:-utf8}',"

    php_string+=$'\n            '
    php_string+="'timezone' => '${COMANAGE_REGISTRY_DATABASE_ENCODING:-UTC}',"

    # Only used when the database driver is Postgres.
    if [[ ((${COMANAGE_REGISTRY_DATABASE_DRIVER} == 'Cake\Database\Driver\Postgres') ||
         -z ${COMANAGE_REGISTRY_DATABASE_DRIVER}) &&
         -n ${COMANAGE_REGISTRY_DATABASE_SCHEMA} ]]; then
        php_string+=$'\n            '
        php_string+="'schema' => '${COMANAGE_REGISTRY_DATABASE_SCHEMA}',"
    fi

    if [[ -n ${COMANAGE_REGISTRY_DATABASE_UNIX_SOCKET} ]]; then
        php_string+=$'\n            '
        php_string+="'unix_socket' => '${COMANAGE_REGISTRY_DATABASE_UNIX_SOCKET}',"
    fi

    # Only used when the database driver is MySQL.
    if [[ (${COMANAGE_REGISTRY_DATABASE_DRIVER} == 'Cake\Database\Driver\Mysql') &&
          -n ${COMANAGE_REGISTRY_DATABASE_SSL_KEY} ]]; then
        php_string+=$'\n            '
        php_string+="'ssl_key' => '${COMANAGE_REGISTRY_DATABASE_SSL_KEY}',"
    fi

    # Only used when the database driver is MySQL.
    if [[ (${COMANAGE_REGISTRY_DATABASE_DRIVER} == 'Cake\Database\Driver\Mysql') &&
          -n ${COMANAGE_REGISTRY_DATABASE_SSL_CERT} ]]; then
        php_string+=$'\n            '
        php_string+="'ssl_cert' => '${COMANAGE_REGISTRY_DATABASE_SSL_CERT}',"
    fi

    # Only used when the database driver is MySQL.
    if [[ (${COMANAGE_REGISTRY_DATABASE_DRIVER} == 'Cake\Database\Driver\Mysql') &&
          -n ${COMANAGE_REGISTRY_DATABASE_SSL_CA} ]]; then
        php_string+=$'\n            '
        php_string+="'ssl_ca' => '${COMANAGE_REGISTRY_DATABASE_SSL_CA}',"
    fi

    php_string+=$'\n            '
    php_string+="'log' => ${COMANAGE_REGISTRY_DATABASE_LOG:-false},"

    php_string+=$'\n            '
    php_string+="'quoteIdentifiers' => ${COMANAGE_REGISTRY_DATABASE_QUOTE_IDENTIFIERS:-false},"
...
EOF
    php_string+=$'\n        '
    php_string+="'transport' => '${COMANAGE_REGISTRY_EMAIL_TRANSPORT:-Smtp}',"
    ...
    # The value of timeout is an integer.
    if [[ -n "${COMANAGE_REGISTRY_EMAIL_TIMEOUT}" ]]; then
        php_string+=$'\n        '
        php_string+="'timeout' => ${COMANAGE_REGISTRY_EMAIL_TIMEOUT},"
    fi
    ...
    # The value of tls is a PHP boolean.
    php_string+=$'\n        '
    php_string+="'tls' => ${COMANAGE_REGISTRY_EMAIL_TLS:-true},"
    ...
    if [[ -n "${COMANAGE_REGISTRY_EMAIL_FROM_EMAIL}" && -n "${COMANAGE_REGISTRY_EMAIL_FROM_NAME}" ]]; then
        php_string+="'from' => ['${COMANAGE_REGISTRY_EMAIL_FROM_EMAIL}' => '${COMANAGE_REGISTRY_EMAIL_FROM_NAME}'],"
...
function comanage_utils::registry_clear_cache() {
    ...
    ./bin/cake cache clear_all > "$OUTPUT" 2>&1
    ./bin/cake schema_cache clear > "$OUTPUT" 2>&1

    echo "Cleared COmanage Registry CakePHP cache files" > "$OUTPUT"
...
function comanage_utils::registry_setup() {
    ...
    pushd "$COMANAGE_REGISTRY_DIR/app" > "$OUTPUT" 2>&1
    echo "Running ./bin/cake database..." > "$OUTPUT" 2>&1
    ./bin/cake database > "$OUTPUT" 2>&1

    # The setup command will create a salt file if the SECURITY_SALT
    # environment variable is not set so we do that first.
    if [[ -n "$COMANAGE_REGISTRY_SECURITY_SALT" ]]; then
...
EOF
            ...
            if [[ -n "${COMANAGE_REGISTRY_OIDC_PASS_CLAIMS_AS}" ]]; then
                cat >> $virtual_host_config <<EOF
OIDCPassClaimsAs ${COMANAGE_REGISTRY_OIDC_PASS_CLAIMS_AS}
EOF
            fi
...
function comanage_utils::wait_database_connectivity() {
    ...
    until ./bin/cake test -t database > "$OUTPUT" 2>&1; do
