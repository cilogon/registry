...
{
  /**
   * Execute the Groups health SQL and print a formatted report.
   *
  ...
  {
    $io->out('Running Groups health check (AR-Group-9)...');
    $sql = RawSqlQueries::STANDARD_GROUP_ARG9_SQL_QUERY;

    try {
      $rows = $inconn->fetchAllAssociative($sql);
    } catch (\Throwable $e) {
      $io->err('Groups health check failed: ' . $e->getMessage());
      return;
    }
